# Codex Lane Instruction — Lane 12 — Deeplink Action Mapping (v17, mapping-only LoRA)

## Non‑negotiable inputs (read, then follow exactly)
Use ONLY:
1) `Full_Dataset_Spec_FULL_LATEST_v17.md` — lane #12 section + CT sample row logic
2) `MASTER_GLOBAL_SCHEMA_LABELS_SUPERSEDED_v2.md` — canonical enums (especially `deeplink_action` allowed values)
...
This lane teaches Dino to map a deeplink-required user request to **exactly one** `deeplink_action` label.
...
From v17 lane #12 (hard requirements):
...
- `deeplink_action` → REQUIRED (one of the canonical allowed labels; exact string)
...
### Constraints
- Output ONLY the label field (`deeplink_action`).
...
From `MASTER_GLOBAL_SCHEMA_LABELS_SUPERSEDED_v2.md`, extract the full allowed enum list for `deeplink_action`.

Then create a dict-slot bank `action_catalog` where **each entry couples**:
- `deeplink_action` (exact enum string)
...
3) Every row has `deeplink_action` and it is a canonical enum string
