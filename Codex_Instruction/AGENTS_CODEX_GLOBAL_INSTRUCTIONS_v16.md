# AGENTS — Codex Global Instructions (lane.yaml generation) — v17/v2 locked

**DO NOT IMPROVISE. DO NOT TOUCH TOOL CODE.**
This AGENTS file is for a Codex whose only job is: **create/modify per-lane `lane.yaml` files** so that the dino-ds tool generates **only v16-golden rows** and passes the v16 validator gates.
...
```bash
DINO_DS_SKIP_OLLAMA=1 ./scripts/run.sh gate lane --config <LANE_DIR>/lane.yaml --limit 5
```

It runs:
1) validate lane.yaml against `lane_schema.v1.json`
...
## How to author a lane.yaml that won’t underfill
...
1) Author **`lane.yaml` for English** first and make it pass `gate lane --limit 5`.
