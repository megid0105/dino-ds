#DTAD Lane QC Handoff v17r3 (Authoritative)
**Title**: DTAD_Lane_QC_Handoff_v17r3.md
**Author**: DTAD-CTv2
...
For each lane, the dataset is produced as 14 lane_*.yaml files:
lane_en.yaml (default en)
13 duplicates:
lane_zh-hk.yaml
lane_th.yaml
lane_zh-hant.yaml
...
The exact similarity thresholds the lane uses (from v17 + lane config)
...
Each lane produces **14 lane_*.yaml files**:

* `lane_en.yaml` (en) + 13 (`lane_zh-hk.yaml`, `lane_th.yaml`, …)
...
3. `gate lane` PASS/FAIL + the exact lane config thresholds used (per language file)
...
The exact lane config (lane_<lang>.yaml) or, at minimum: similarity settings + structure cap settings + quotas
